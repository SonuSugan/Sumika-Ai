...
export default function App() {
  ...
  const handleFinalText = useCallback(async (text) => {
    ...
    try {
      ...
      // result.reply was generated by the LLM assuming the tool call succeeded
      // ("LinkedIn is open") - true for a typed command (a click IS a user
      // gesture) but false for a voice command hitting the block above. Saying
      // that out loud when it's actually still waiting on a tap is worse than
      // saying nothing, so swap in an honest message and a distinct sound
      // instead of quietly reusing the LLM's (wrong, in this case) reply.
      ...
      if (blocked.length) {
        const names = blocked
          ...
          .join(' and ');
        setPendingAction({ links: blocked, label: `Tap to open ${names} - one-time browser requirement` });
        reply = `I can't open a new tab from a voice command alone - tap the button at the top of the screen to open ${names}. After that one tap, voice will open it directly every time.`;
        sfx.playAlert();
      } else {
        sfx.playReply();
      }

      setMessages((prev) => [...prev, { role: 'assistant', text: reply, provider: result.provider, links }]);
      speak(reply);
      setState('speaking');
    } catch (err) {
      sfx.playAlert();
